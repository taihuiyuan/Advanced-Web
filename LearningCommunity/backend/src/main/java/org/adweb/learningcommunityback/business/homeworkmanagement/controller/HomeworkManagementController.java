...
import org.adweb.learningcommunityback.business.homeworkmanagement.dto.MyHomeworkDTO;
import org.adweb.learningcommunityback.business.homeworkmanagement.dto.ScoreDTO;
...
import org.adweb.learningcommunityback.business.homeworkmanagement.request.GiveScoreRequest;
...
public class HomeworkManagementController {
    ...
    @PreAuthorize("hasAnyAuthority('Teacher') || hasAnyAuthority('TA') || hasAnyAuthority('Student')")
    public List<MyHomeworkDTO> getMyHomework(@AuthenticationPrincipal String username){
        return homeworkManagementService.getMyHomework(username);
    }

    //教师给同学打分
    @PostMapping("/giveScore")
    @PreAuthorize("hasAnyAuthority('Teacher')")
    public SimpleSuccessfulPostResponse giveScore(@RequestBody GiveScoreRequest request,
                                                  @AuthenticationPrincipal String username){
        return homeworkManagementService.giveScore(username, request.getUsername(), request.getCourseCode(), request.getScore());
    }

    //查看评分
    @GetMapping("/scores")
    @PreAuthorize("hasAnyAuthority('Teacher') || hasAnyAuthority('TA') || hasAnyAuthority('Student')")
    public List<ScoreDTO> getScores(@RequestParam("courseCode") String courseCode,
                                    @AuthenticationPrincipal String username){
        return homeworkManagementService.getScores(username, courseCode);
    }
}
